        ...
        for i in range(len(VOCAB)):
            if i == self.mask_idx or i == self.unk_idx:
                ...
                if not backbone_only:
                    ...
                    sidechain_atoms = const.sidechain_atoms.get(base_symbol, const.sidechain_atoms.get(symbol, []))
            ...
            for i in range(len(VOCAB)):
                if special_mask[i] == 1:
                    ...
                    if hasattr(VOCAB, 'resolve_symbol'):
                        ...
                        base_symbol = getattr(VOCAB, 'alias_symbol', {}).get(symbol, symbol)
                    atom_type = VOCAB.backbone_atoms + const.sidechain_atoms.get(base_symbol, const.sidechain_atoms.get(symbol, []))
                    atom2channel = { atom: i for i, atom in enumerate(atom_type) }
                    chi_key = VOCAB.symbol_to_abrv(base_symbol)
                    chi_atoms = const.chi_angles_atoms.get(chi_key, [])
                    bond_atoms = const.sidechain_bonds.get(base_symbol, const.sidechain_bonds.get(symbol, []))
                    sc_chi_atoms.append(
