...
class AutoEncoder(nn.Module):
    def __init__(
            ...
            enable_enlarged_vocab=False,
            enable_latent_anchor_channels=False,
            latent_anchor_atoms=None,
        ...
        self.enable_latent_anchor_channels = enable_latent_anchor_channels
        self.latent_anchor_atoms = latent_anchor_atoms
        self.latent_anchor_pad_atom = latent_anchor_pad_atom
        self.latent_anchor_strict = latent_anchor_strict
        ...
        # residue type index mapping, from original index to trainable classes, 0 is unk
        self.unk_idx = 0
        if self.enable_enlarged_vocab:
            trainable = [i for i, m in enumerate(VOCAB.get_special_mask()) if m == 0]
            self.s_remap = [VOCAB.symbol_to_idx(VOCAB.UNK)] + trainable
            self.s_map = [0 for _ in range(len(VOCAB))]
            for class_idx, vocab_idx in enumerate(self.s_remap):
                self.s_map[vocab_idx] = class_idx
        else:
            self.s_map = [0 for _ in range(len(VOCAB))]
        ...
        self.anchor_channel_map = None
        self.anchor_pad_channel = None
        self.anchor_pad_latent_idx = None
        if self.enable_latent_anchor_channels:
            self._build_anchor_channel_map()
    ...
    def _get_latent_channels(self, X, atom_mask, S=None):
        ...
        if self.enable_latent_anchor_channels:
            return self._get_anchor_latent_channels(X, atom_mask, S)
    ...
    def _get_latent_channel_anchors(self, X, atom_mask, S=None):
        if self.anchor_at_ca:
            return X[:, self.ca_channel_idx].unsqueeze(1)
        if self.enable_latent_anchor_channels:
            return self._get_anchor_latent_channels(X, atom_mask, S)
        else:
            return self._get_latent_channels(X, atom_mask, S)
        
    def _fill_latent_channels(self, latent_X, S=None):
        if self.enable_latent_anchor_channels:
            return self._fill_anchor_latent_channels(latent_X, S)
    ...
    def _build_anchor_channel_map(self):
        if self.latent_anchor_atoms is None:
            self.latent_anchor_atoms = {
                'backbone': ['N', 'CA', 'C'],
                'ss': ['SG'],
                'iso_lys': ['NZ'],
                'iso_asp': ['CG'],
        ...
        cfg = self.latent_anchor_atoms
        backbone = cfg.get('backbone', ['N', 'CA', 'C'])
        ss_atoms = cfg.get('ss', [])
        iso_lys = cfg.get('iso_lys', [])
        iso_asp = cfg.get('iso_asp', cfg.get('iso_acid', []))
        iso_glu = cfg.get('iso_glu', cfg.get('iso_acid', []))
        pad_atom = self.latent_anchor_pad_atom or 'CA'

        anchor_channel_map = []
        pad_channel = []
        pad_latent_idx = []
        for idx in range(len(VOCAB)):
            symbol = VOCAB.idx_to_symbol(idx)
            if hasattr(VOCAB, 'resolve_symbol'):
                base_symbol = VOCAB.resolve_symbol(symbol)
            else:
                base_symbol = getattr(VOCAB, 'alias_symbol', {}).get(symbol, symbol)
            atom_list = VOCAB.backbone_atoms + const.sidechain_atoms.get(base_symbol, [])
            extras = []
            if base_symbol == 'C':
                extras = ss_atoms
            elif base_symbol == 'K':
                ...
                extras = iso_glu
            anchors = backbone + extras
            if len(anchors) > self.latent_n_channel:
                msg = f'anchor atoms {anchors} exceed latent_n_channel={self.latent_n_channel}'
                if self.latent_anchor_strict:
                    raise ValueError(msg)
                print_log(f'WARNING: {msg}, truncating', level='WARN')
                anchors = anchors[: self.latent_n_channel]
            if len(anchors) < self.latent_n_channel:
                anchors = anchors + [pad_atom] * (self.latent_n_channel - len(anchors))

            channel_idx = []
            for atom in anchors:
                if atom in atom_list:
                    channel_idx.append(atom_list.index(atom))
                else:
                    channel_idx.append(-1)
            anchor_channel_map.append(channel_idx)

            if pad_atom in atom_list:
                pad_channel.append(atom_list.index(pad_atom))
            elif 'CA' in atom_list:
                pad_channel.append(atom_list.index('CA'))
            else:
                pad_channel.append(0)

            if pad_atom in anchors:
                pad_latent_idx.append(anchors.index(pad_atom))
            else:
                pad_latent_idx.append(0)

        self.anchor_channel_map = torch.tensor(anchor_channel_map, dtype=torch.long)
        self.anchor_pad_channel = torch.tensor(pad_channel, dtype=torch.long)
        self.anchor_pad_latent_idx = torch.tensor(pad_latent_idx, dtype=torch.long)

    def _get_anchor_latent_channels(self, X, atom_mask, S):
        if S is None:
            if self.latent_anchor_strict:
                raise ValueError('latent anchor channels enabled but S is None')
            anchor = X[:, self.ca_channel_idx].unsqueeze(1)
            return anchor.repeat(1, self.latent_n_channel, 1)
        if self.anchor_channel_map is None:
            self._build_anchor_channel_map()

        S = S.long()
        rows = torch.arange(X.size(0), device=X.device)
        anchor_idx = self.anchor_channel_map.to(S.device)[S]  # [N, K]
        pad_channel = self.anchor_pad_channel.to(S.device)[S]
        pad_coord = X[rows, pad_channel]

        coords = []
        for k in range(anchor_idx.size(1)):
            idx = anchor_idx[:, k]
            valid = (idx >= 0) & (idx < X.size(1))
            idx_safe = idx.clamp(min=0, max=X.size(1) - 1)
            coord = X[rows, idx_safe]
            if atom_mask is not None:
                valid = valid & atom_mask[rows, idx_safe]
            coord = torch.where(valid.unsqueeze(-1), coord, pad_coord)
            coords.append(coord)
        return torch.stack(coords, dim=1)

    def _fill_anchor_latent_channels(self, latent_X, S):
        if S is None:
            if self.latent_anchor_strict:
                raise ValueError('latent anchor channels enabled but S is None')
            return latent_X[:, :1].repeat(1, self.n_channel, 1)
        if self.anchor_channel_map is None:
            self._build_anchor_channel_map()

        S = S.long()
        rows = torch.arange(latent_X.size(0), device=latent_X.device)
        anchor_idx = self.anchor_channel_map.to(S.device)[S]  # [N, K]
        pad_latent_idx = self.anchor_pad_latent_idx.to(S.device)[S]
        pad_latent_idx = pad_latent_idx.clamp(min=0, max=latent_X.size(1) - 1)
        fill_coord = latent_X[rows, pad_latent_idx]
        X_full = fill_coord[:, None, :].repeat(1, self.n_channel, 1)

        for k in range(anchor_idx.size(1)):
            idx = anchor_idx[:, k]
            valid = (idx >= 0) & (idx < self.n_channel)
            if valid.any():
                X_full[rows[valid], idx[valid]] = latent_X[rows[valid], k]
        return X_full
    ...
    def encode(self, X, S, mask, position_ids, lengths, atom_mask, no_randomness=False):
        ...
        if self.mode != 'fixbb':
            if hasattr(self, 'fix_alpha_carbon') and self.fix_alpha_carbon:
                Z = self._get_latent_channels(true_X, atom_mask, S)
            else:
                Z = self._get_latent_channels(pred_X, atom_mask, S)
            Z_centers = self._get_latent_channel_anchors(true_X, atom_mask, S)
    ...
    def decode(self, X, S, H, Z, mask, position_ids, lengths, atom_mask, teacher_forcing):
        ...
        if self.mode != 'fixbb':  # fill coordinates with latent points
            ...
            if teacher_forcing:
                S_fill[mask] = true_S
            if self.enable_latent_anchor_channels:
                X[mask] = self._fill_latent_channels(Z, S_fill[mask])
            else:
                X[mask] = self._fill_latent_channels(Z, S_fill)
    ...
    def forward(self, X, S, mask, position_ids, lengths, atom_mask, teacher_forcing=True):
        ...
        if self.mode != 'fixbb':
            coord_reg_loss = F.mse_loss(Z, self._get_latent_channel_anchors(true_X, atom_mask[mask], true_S))
        else:
            coord_reg_loss = 0
